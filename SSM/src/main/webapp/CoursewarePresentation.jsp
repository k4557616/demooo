...
<form action="videoshow">
        <!--面包屑导航-->
        <div class="container mian-nav" id="navDiv"><!-- 公开课 / WEB前端 --> 
        
        总共${info.total }条数据,共${info.pages }页,当前第${info.pageNum }页
        <a href="videoshow?n=1">首页</a>
       <c:if test="${info.pageNum>1 }">
       <a href="videoshow?n=${info.prePage }&title=${name2}">上一页</a>
       </c:if>
       <c:forEach items="${info.navigatepageNums }" var="a">
       <a href="videoshow?n=${a }">${a }</a>
       </c:forEach>
       <c:if test="${info.pageNum<=info.pages-1 }">
       <a href="videoshow?n=${info.nextPage }&title=${name2}">下一页</a>
       </c:if>
       <a href="videoshow?n=${info.pages }">尾页</a>
        <input type="text" name="title" value="${name2 }" placeholder="请输入视频标题" >
