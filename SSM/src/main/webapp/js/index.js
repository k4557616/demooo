...
$('#login_closee').click(function(){
    $('#login').addClass('hidden');
});

//管理员
$('#adminxian').click(function () {
	alert(hh);
    $('#loginadmin2').removeClass('hidden');
});
$('#adminxiao').click(function(){
    $('#loginadmin2').addClass('hidden');
});
...
/*function commitLogin(){
   ...
   var email =$("#loginEmaill").val();
   var password =$("#loginPasswordd").val();
  
   if(null!=email && email!="" && null!=password && password!=""){
	   alert(11);
        var params=$("#loginFormm").serialize(); 
        alert(params);
        // post要小写
        
        $.post("loginuseryan",params,function(data){
        // alert(data);
